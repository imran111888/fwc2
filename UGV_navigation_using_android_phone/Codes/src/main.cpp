...
void loop() {
  _loop();
  if (GamePad.isPressed(3)) { //Move forward
    Motor1.moveMotor(100);
    Motor2.moveMotor(100); 
  }
  else {
    if (GamePad.isPressed(2)) { //Move backward
      Motor1.moveMotor(-100);
      Motor2.moveMotor(-100);
    }
    else {
      if (GamePad.isPressed(0)) { //Move right
        Motor1.moveMotor(100);
        Motor2.moveMotor(-100);
      }
      else {
        if (GamePad.isPressed(1)) { //Move left
          Motor1.moveMotor(-100);
          Motor2.moveMotor(100);
        }
        else {
